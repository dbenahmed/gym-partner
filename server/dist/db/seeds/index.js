...
const main = async () => {
    await db.transaction(async (tx) => {
        try {
            ...
            //console.log('here 1')
            const exercisesMock = await Promise.all(Array.from({ length: 10 }, async (_, index) => {
                try {
                    return ({
            ...
            const setsOfSessionsExercisesMock = await Promise.all(Array.from({ length: 10 }, async (_, index) => {
                try {
            ...
            const foodsLogsMock = await Promise.all(Array.from({ length: 10 }, async (_, index) => {
                try {
            ...
            const weightsLogsMock = await Promise.all(Array.from({ length: 10 }, async (_, index) => {
                try {
                    const userId = insertedUsers[index % insertedUsers.length]?.id; // Assign userId from existing users
                    return {
                        userId,
