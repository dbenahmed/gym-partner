...
import { exercises, plansExercises, sessions, setsOfSessionsExercises, } from "../db/schemas/schema.js";
...
export const createWorkoutSession = async (req, res) => {
    try {
        ...
        const { planId, dueDate, name, startTime, endTime, note, rating, exercisesArray, } = req.body;
        console.log("backend");
        ...
        const finished = await db.transaction(async (tx) => {
            ...
            const exercisesValidation = await Promise.all(exercisesArray.map(async (exo) => {
                ...
                if (weights.length !== units.length ||
...
export const saveExerciseToSession = async (req, res) => {
    try {
        ...
        const existExercise = await db.query.sessionExercises.findFirst(where(eq(sessionExercises.id, Id)));
