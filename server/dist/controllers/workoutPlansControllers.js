...
export const createWorkoutCollection = async (req, res) => {
    try {
        ...
        const exists = await db.select().from(collections).where(and(eq(collections.title, title), eq(collections.userId, user.id))).limit(1);
        if (exists.length > 0) {
            res.status(400).json({
                success: false,
...
export const updateWorkoutCollection = async (req, res) => {
    try {
        ...
        const collectionExists = await db.select().from(collections).where(and(eq(collections.id, collectionId), eq(collections.userId, user.id))).limit(1);
...
export const deleteWorkoutCollection = async (req, res) => {
    try {
        ...
        const collectionExists = await db.select().from(collections).where(and(eq(collections.id, collectionId), eq(collections.userId, userId))).limit(1);
        if (collectionExists.length === 0) {
...
export const getWorkoutPlans = async (req, res) => {
    try {
        ...
        const foundCollections = await db.select().from(collections).where(and(eq(collections.id, collectionId), eq(collections.userId, userId))).limit(1);
...
export const createWorkoutPlan = async (req, res) => {
    try {
        ...
        const collectionsFound = await db.select().from(collections).where(and(eq(collections.id, collectionId), eq(collections.userId, userId))).limit(1);
        ...
        const foundPlans = await db.select().from(plans).where(and(eq(plans.title, title), eq(plans.collectionId, collectionId)));
...
export const addExerciseToPlan = async (req, res) => {
    try {
        ...
        for (const exercise of foundExercises) {
            const exerciseNotAlreadyAdded = await db.select().from(plansExercises).where(and(eq(plansExercises.planId, planId), eq(plansExercises.exerciseId, exercise.id))).limit(1);
...
export const removeExerciseFromPlan = async (req, res) => {
    try {
        ...
        const exerciseNotAlreadyAdded = await db.select().from(plansExercises).where(and(eq(plansExercises.planId, planId), eq(plansExercises.exerciseId, exerciseId))).limit(1);
        if (exerciseNotAlreadyAdded.length = 0) {
            res.status(401).json({
                success: false,
        ...
        await db.delete(plansExercises).where(and(eq(plansExercises.planId, planId), eq(plansExercises.exerciseId, exerciseId)));
        res.json({
            success: true,
