...
export const findPlanWithOwnership = async (userId: number, planId: number) => {
    return db.select()
        .from(plans)
...
export const findPlansByCollectionId = async (collectionId: number) => {
    return db.select({
        id: plans.id,
...
export const findPlanByTitleAndCollectionId = async (title: string, collectionId: number) => {
    return db.select().from(plans).where(
        and(eq(plans.title, title), eq(plans.collectionId, collectionId))
    );
};

export const createPlan = async (title: string, collectionId: number) => {
...
export const updatePlan = async (planId: number, newData: Partial<{ title: string }>) => {
...
export const findPlanWithCollection = async (planId: number, userId: number) => {
...
export const findExerciseInPlan = async (planId: number, exerciseId: number) => {
    return db.select().from(plansExercises).where(
        and(eq(plansExercises.planId, planId), eq(plansExercises.exerciseId, exerciseId))
    ).limit(1);
};

export const createPlanExercise = async (planId: number, exerciseId: number, order: number) => {
...
export const deletePlanExercise = async (planId: number, exerciseId: number) => {
