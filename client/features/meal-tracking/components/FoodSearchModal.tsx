...
import { validateName, validateAllNutrition } from "@/utils/validation";

export default function FoodSearchModal({ isVisible, onClose, onFoodSelect, onCustomFoodPress }: any) {
  ...
  const searchForFood = async (text: any) => {
    ...
    if (success) {
      const nonRefusedMeals = meals.filter(
        (meal: any) => meal.status !== "refused" || meal.createdBy === userId
      );
      ...
      <View style={{ flex: 1 }}>
        ...
        {searchForFoodLoading ? (
          ...
          <FlatList
            data={
              verifiedOnly
                ? foods.filter((item: any) => item.status === "verified")
                : mine
                  ? foods.filter((item: any) => item.createdBy === userId)
                  : foods
            }
