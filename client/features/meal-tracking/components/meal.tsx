...
export default function Meal({ data, onDelete, onUpdate }: any) {


  // styles
  let { theme, colors } = useThemeContext();
  if (!colors) {
    console.warn("Colors not defined in theme context. Using default colors.");
    colors = (Color as any)[theme] || Color.light; // Fallback to light theme if colors are not defined
  }
  if (!theme) {
  ...
  const calculateNutrient = (valuePer100g: any, servingSize: any) => {
    return ((valuePer100g * servingSize) / 100).toFixed(1);
  };

  const calculateCalories = (caloriesPer100g: any, servingSize: any) => {
