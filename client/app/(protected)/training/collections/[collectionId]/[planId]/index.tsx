...
import { fetchGetPlanExercises, fetchAddExerciseToPlan, fetchSearchExercises, fetchDeletePlan, fetchCreateExercise } from '@/lib/api'; // Replace with real path
...
const Exercises = () => {
  ...
  const handleSearchExercise = async (query: any) => {
    if (query.length === 0) {
      setSearchResults([]);
      setCount(0);
      return;
  ...
  const { collectionId, planId, title } = useLocalSearchParams() as { collectionId: string, planId: string, title?: string };

  const handleLimitChanged = async (newLimit: any) => {
    setLimit(newLimit);
