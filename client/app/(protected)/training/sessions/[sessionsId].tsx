...
export default function SessionDetails() {
  ...
  const { sessionsId } = useLocalSearchParams() as { sessionsId: string };
            ...
            <View style={styles.statItem}>
              ...
              <Text style={styles.statValue}>
                {session.starttime && session.endtime
                  ?
                  `${Math.floor((new Date(session.endtime).getTime() - new Date(session.starttime).getTime()) / 3600000)
                    .toString()
                    .padStart(2, '0')}:${Math.floor(((new Date(session.endtime).getTime() - new Date(session.starttime).getTime()) / 60000) % 60)
                      .toString()
                      .padStart(2, '0')}:${Math.floor(((new Date(session.endtime).getTime() - new Date(session.starttime).getTime()) / 1000) % 60)
                        .toString()
                        .padStart(2, '0')}`
                  : "N/A"}
              </Text>
              ...
              exercises.map((exercise: any, index: any) => (
                <View key={exercise.id} style={styles.exerciseCard}>
                  ...
                  <View style={styles.setsContainer}>
                    {exercise.weight.map((weight: any, setIndex: any) => (
