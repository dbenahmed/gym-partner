...
export default function StartSession() {
   ...
   const [duration, setDuration] = useState<any>(null) // Duration as a date;
                  ...
                  [
                     ...
                     },
                     {
                        ...
                        onPress: () => {
                           ...
                           console.log("----------------------------")
                           const newDuration = new Date().getTime() - new Date(extractedData.startTime).getTime()
   ...
   useEffect(() => {
      ...
      const interval = setInterval(() => {
         const newDuration = new Date().getTime() - startTime.current.getTime();
         setDuration(newDuration);
      }, 1000);
   ...
   const addManyExercisesToSession = (exercisesArray: any[]) => {
      const newExercises = exercisesArray.map((exercise: any) => ({
      ...
      const filteredExercises = newExercises.filter((exercise: any) => !existingExerciseIds.includes(exercise.id));
   ...
   const addExerciseToSession = async (exercise: any) => {
      try {
         ...
         if (Array.isArray(data) && data.length > 0) {
            ...
            filtered = data.sort((a: any, b: any) => new Date(b.creationdate).getTime() - new Date(a.creationdate).getTime())
            const latest = data[0]
   ...
   const updateExerciseData = (id: any, field: any, value: any) => {
   ...
   const searchExercises = async (query: any) => {
